...
function renderMalla(userId, aprobados = []) {
  ...
  ramos.forEach(ramo => {
    ...
    div.className = "ramo" + (aprobados.includes(ramo.id) ? " approved" : "");
    div.id = ramo.id;
    div.style.backgroundColor = ramo.color;
    div.innerHTML = `<strong>${ramo.nombre}</strong>`;
    div.onclick = () => toggleRamo(ramo.id, userId);
    malla.appendChild(div);
  });
  actualizarMalla();
}

// Alternar estado aprobado/no aprobado y guardar
function toggleRamo(id, userId) {
...
// Activar/desactivar ramos según requisitos
...
// Guardar progreso del usuario en Firebase
function guardarEstado(userId) {
  const aprobados = ramos
    .filter(r => document.getElementById(r.id)?.classList.contains("approved"))
    .map(r => r.id);
  db.collection("usuarios").doc(userId).set({ aprobados });
}

// Cargar progreso del usuario desde Firebase
function cargarEstado(userId) {
  db.collection("usuarios").doc(userId).get().then(doc => {
    const aprobados = doc.exists ? doc.data().aprobados : [];
